...
<script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>" src="<?php p($urlGenerator->linkTo('files_ifcviewer', 'js/viewer/iframe-communications.js')) ?>?v=<?php p($version) ?>"></script>
    <?php if($_['minmode'] || true ):?>
		<script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>" src="<?php p($urlGenerator->linkTo('files_ifcviewer', 'js/viewer/iframe-main-min.js')) ?>?v=<?php p($version) ?>"></script>
    <?php else: ?>
		<script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>" src="<?php p($urlGenerator->linkTo('files_ifcviewer', 'js/viewer/iframe-main.js')) ?>?v=<?php p($version) ?>"></script>
   <?php endif;?>
